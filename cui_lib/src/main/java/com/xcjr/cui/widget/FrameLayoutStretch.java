 ...
 * version: 1.0.0
 */
public class FrameLayoutStretch extends FrameLayout implements View.OnTouchListener {

    static final int ANIMATED_SCROLL_GAP = 250;
    static final float MAX_SCROLL_FACTOR = 0.5f;
    static final float OVERSHOOT_TENSION = 0.75f;
    static final int STRETCH_MAX_TOP = 1500;
    static final int STRETCH_MAX_BOTTOM = 1500;
    private long mLastScroll;
    private int stretchMaxTop, stretMaxBottom;
    ...
    public FrameLayoutStretch(Context context) {
        this(context, null, 0);
    }

    public FrameLayoutStretch(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
        initBounce();
    }

    public FrameLayoutStretch(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void initChildPointer() {
        child = getChildAt(0);
        child.setPadding(0, STRETCH_MAX_TOP, 0, STRETCH_MAX_BOTTOM);

    }

    public void setMaxStretchTop(int stretchOffset) {
        if (child != null) {
            child.setPadding(child.getPaddingLeft(), stretchOffset, child.getPaddingRight(), child.getPaddingBottom());
        }
    }

    public void setMaxStretchBottom(int stretchOffset) {
        if (child != null) {
            child.setPadding(child.getPaddingLeft(), child.getPaddingTop(), child.getPaddingRight(), stretchOffset);
        }
    }
     ...
     *                  to go one page up or {@link View#FOCUS_DOWN} to go
     *                  one page down
     ...
     *
     * @param direction the scroll direction: {@link View#FOCUS_UP}
     *                  to go the top of the view or {@link View#FOCUS_DOWN}
     *                  to go the bottom
     * @return true if the key event is consumed by this method, false otherwise
     */
